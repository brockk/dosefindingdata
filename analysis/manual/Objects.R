...
cache_dir <- here('analysis', 'manual', '.cache')
# dlt_series = dlt %>% pull(AnalysisSeriesId) %>% unique() %>% head(50)
# obj_resp_series = obj_resp %>% pull(AnalysisSeriesId) %>% unique() %>% head(50)
dlt_series = dlt %>% pull(AnalysisSeriesId) %>% unique()
obj_resp_series = obj_resp %>% pull(AnalysisSeriesId) %>% unique()

# Cache management----

# Function retrieves a named object from the cache.
# Returns a list with elements 'object' and 'present', reflecting whether an 
# object was present or not.
.cache_retrieve <- function(object_name) {
  object_path <- paste0(cache_dir, '/', object_name, '.RDS')
  if(file.exists(object_path)) {
    obj <- readRDS(object_path)
    return(list(object = obj, present = TRUE))
  } else {
    return(list(object = NULL, present = FALSE))
  }
}

.cache_save <- function(object, object_name) {
  object_path <- paste0(cache_dir, '/', object_name, '.RDS')
  saveRDS(object, object_path)
}
# object_name <- 'dlt_model_sig_df_1'
# .cache_retrieve(object_name)
# .cache_save(NULL, object_name)
# .cache_retrieve(object_name)
...
# In the following functions, if object_name is given, the code will search
# the cache first, only run code to create new objects when no cached object is 
# available, and save to the cache any newly-created objects. 

fit_sig_df_model <- function(df, object_name = NULL) {
  
  if(!is.null(object_name)) {
    x <- .cache_retrieve(object_name)
    if(x$present) return(x$object)
  } 
  
  # Only try to fit model when events have occurred.
  if(sum(df$Event) > 0) {
    model <- fitMod(DoseLevelN, Event, data = df,  model = "sigEmax",
                    bnds = defBnds(mD = max(df$DoseLevelN))$sigEmax)
  } else {
    # Use NULL for trivial model.
    model <- NULL
  }
  
  if(!is.null(object_name)) .cache_save(model, object_name)
  return(model)
}

# Standard parametisation of EMax model
.fit_type1_brms_model <- function(df, prior, seed, object_name = NULL) {
  ...
  if(sum(df$Event) > 0)
  {
    ...
    model <- brm(form, prior = prior, data = df, cores = 4, 
...
fit_type1_brms_model_p1 <- function(df, seed, object_name = NULL) {
  ...
  .fit_type1_brms_model(df, prior, seed, object_name)
}

fit_type1_brms_model_p2 <- function(df, seed, object_name = NULL) {
  prior_mean_ed50 <- max(df$DoseLevelN) / 2
...
# Adjusted parametisation of EMax model for easier prior specifiation
.fit_type2_brms_model <- function(df, prior, seed, object_name = NULL) {
...
fit_type2_brms_model_p1 <- function(df, seed, object_name = NULL) {
  prior_mean_ed50 <- max(df$DoseLevelN) / 2
  prior_sd_ed50 <- (max(df$DoseLevelN) - min(df$DoseLevelN)) / 2
  ed50_prior_str <- paste0('normal(', prior_mean_ed50, ', ', prior_sd_ed50, ')')
  prior <- set_prior('beta(1, 1)', nlpar = "E0", lb = 0, ub = 1) + 
    ...
    set_prior(ed50_prior_str, nlpar = "ED50", lb = 0) +
    set_prior('normal(1, 3)', nlpar = "N", lb = 0)
  .fit_type2_brms_model(df, prior, seed, object_name)
}

fit_type2_brms_model_p2 <- function(df, seed, object_name = NULL) {
  prior_mean_ed50 <- max(df$DoseLevelN) / 2
  prior_sd_ed50 <- (max(df$DoseLevelN) - min(df$DoseLevelN))
  ed50_prior_str <- paste0('normal(', prior_mean_ed50, ', ', prior_sd_ed50, ')')
  prior <- set_prior('beta(1, 1)', nlpar = "E0", lb = 0, ub = 1) + 
...
augment.DRMod <- function(obj, predType = 'full-model', newdata = NULL, ...) {
...
# extract_parameter_estimates <- function(model, object_name) {
#   broom::tidy(model) %>% 
