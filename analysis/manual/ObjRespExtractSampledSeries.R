...
for(series in obj_resp_series) {
  ...
  if(x$present) {
    model <- x$object
    object <- get_fitted_draws(model) %>% 
      mutate(AnalysisSeriesId = series, Model = 'Bayes1P1')
    object_name <- paste0('obj_resp_type1_brms_p1_samples_', series)
  ...
  if(x$present) {
    ...
    object_name <- paste0('obj_resp_type1_brms_p2_samples_', series)
    .cache_save(object, object_name)
  } else {
    warning(paste0('Object ', sought_object_name, ' not present.'))
  }
  ...
  if(x$present) {
    ...
    object_name <- paste0('obj_resp_type2_brms_p1_samples_', series)
    .cache_save(object, object_name)
  } else {
    warning(paste0('Object ', sought_object_name, ' not present.'))
  }
  ...
  if(x$present) {
    ...
    object_name <- paste0('obj_resp_type2_brms_p2_samples_', series)
    .cache_save(object, object_name)
  } else {
    warning(paste0('Object ', sought_object_name, ' not present.'))
  }
